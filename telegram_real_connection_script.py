...
async def send_reklama_message(client, chat_id, user_data, user_id):
    """ Media (Rasm), Inline tugmalar va textni yaxlit bitta xabar sifatida (Caption bilan) yuboruvchi asinxron motor """
    text = user_data.get("reklama_matni", "")
    
    # REAL IMZO TIZIMI (YANGI 🌟)
    # Agar reklama yuborayotgan a'zo admin bo'lmasa, reklama oxiriga maxsus imzo qo'shiladi!
    if int(user_id) != ADMIN_ID:
        text += "\n\n@Auto_Xabar_Yuborish_Bot orqali yuborildi"
        
    photo_path = user_data.get("reklama_rasm") # Mahalliy saqlangan rasm fayl yo'li
    ...
    # Rasm mavjud bo'lsa - rasm + matn (imzosi bilan) bitta xabar sifatida yuboriladi
    if photo_path and os.path.exists(photo_path):
        ...
        # Rasm bo'lmasa faqat matn (imzosi bilan) yuboriladi
        await client.send_message(chat_id, text, buttons=telethon_buttons)

async def auto_sender_worker():
    """ Orqa fonda barcha faol foydalanuvchilar uchun xabarlarni interval asosida tarqatib turuvchi asinxron loop """
    while True:
        current_time = datetime.now().timestamp()
        
        for user_id, user_data in list(db_users.items()):
            # Agar foydalanuvchi yuborishni yoqqan bo'lsa va akkaunti ulangan bo'lsa
            if user_data.get("is_sending") and user_data.get("active_phone"):
                next_run = user_data.get("next_run_timestamp", 0)
                
                # Belgilangan interval vaqti kelgan bo'lsa
                if current_time >= next_run:
                    interval_minutes = user_data.get("interval", 15)
                    # Keyingi sikl vaqtini o'rnatamiz (Hozirgi vaqt + interval daqiqa)
                    db_users[user_id]["next_run_timestamp"] = current_time + (interval_minutes * 60)
                    save_db()
                    
                    # Siklni alohida asinxron vazifa (Task) sifatida ishga tushiramiz
                    asyncio.create_task(run_sending_cycle_for_user(user_id))
                        
        # Sikl vaqtini har 10 soniyada juda faol ravishda tekshirib boradi
        await asyncio.sleep(10)

async def run_sending_cycle_for_user(user_id):
    """ Foydalanuvchiga tegishli guruhlarga xabarlarni guruhlararo kechikish bilan tarqatuvchi yordamchi """
    user_data = db_users.get(user_id)
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            # Agar foydalanuvchi o'zi tanlagan guruhlar bo'lsa
            if choice == "custom":
                guruhlar = [int(x) for x in user_data.get("selected_groups", [])]
            else:
                # Hamma guruhlarni avtomatik yuklaymiz
                async for dialog in client.iter_dialogs():
                    if dialog.is_group:
                        guruhlar.append(int(dialog.id))
            
            if not guruhlar:
                logging.warning(f"[Sender] Foydalanuvchi {user_id} uchun hech qanday guruh topilmadi.")
                return
                
            logging.info(f"[Sender] Foydalanuvchi {user_id} uchun {len(guruhlar)} ta guruhga tarqatish boshlandi...")
            
            # Guruhlarga bitta-bitta xabar yuboramiz
            for g_id in guruhlar:
                # Agar foydalanuvchi yuborishni to'xtatib qo'ygan bo'lsa, zudlik bilan chiqamiz
                if not db_users.get(user_id, {}).get("is_sending"):
                    break
                    
                try:
                    # MUHIM: Foydalanuvchi ID raqamini ham imzo qo'shish uchun argument qilib yuboramiz!
                    ...
                    # Spam-blokdan saqlanish (Xavfsizlik delay: 15 soniya)
...
async def init_existing_sessions():
    admin_session_src = "autohabar_user_session.session"
    admin_session_dst = os.path.join(SESSIONS_DIR, f"session_{ADMIN_ID}.session")
    
    if os.path.exists(admin_session_src) and not os.path.exists(admin_session_dst):
        try:
            shutil.copy(admin_session_src, admin_session_dst)
            logging.info(f"Mavjud admin sessiya fayli '{admin_session_src}' muvaffaqiyatli '{admin_session_dst}' papkasiga ko'chirildi!")
        except Exception as e:
            logging.error(f"Sessiya faylini ko'chirishda xatolik: {e}")
...
async def main():
    ...
    print("\n⚠️ DIQQAT: Akkauntlarni ulash va kod kiritish konsolda emas,")
    print("           to'g'diran-to'g'ri Telegram botingiz ichida bajariladi!")
    print("==================================================")
