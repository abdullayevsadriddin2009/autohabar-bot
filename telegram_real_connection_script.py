...
Sadriddin uchun barcha yangi funksiyalar va suv belgisi (watermark) 100% integratsiya qilingan to'liq versiya!
"""
...
DEFAULT_DB = {
    ADMIN_ID: {
        ...
        "lang": "uz",
        "blacklist": []  # Qora ro'yxat uchun standart maydon
    }
}
...
def ensure_user(user_id: int):
    ...
    if user_id not in db_users:
        db_users[user_id] = {
            ...
            "auto_reply_active": False,
            "lang": None,
            "blacklist": []  # Qora ro'yxat maydoni
        }
    else:
        ...
        if "blacklist" not in db_users[user_id] or db_users[user_id]["blacklist"] is None:
...
class TextStates(StatesGroup):
    ...
    waiting_blacklist_gids = State()   # Qora ro'yxatni tahrirlash holati
...
def get_interval_keyboard(current_interval):
    ...
    kb = [
        ...
        [
            InlineKeyboardButton(text="← Orqaga", callback_data="back_to_welcome")
        ]
    ]
...
LOCALIZATION = {
    "uz": {
        ...
        "guide_text": "📖 <b>AutoHabar Pro - Foydalanish Bo'yicha Oson Qo'llanma</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                      ...
                      "• Guruhlarni keshdan yangilash uchun <b>+ Qo'shish</b> tugmasini bosing, so'ngra xabarlar tarqatiladigan guruhlarni belgilang va saqlang.\n\n"
                      "4️⃣ <b>Interval va Taymer:</b>\n"
                      "• ⏱️ <b>Interval</b> menyusida kutish vaqtini belgilang (tavsiya etiladi: 15 daqiqa).\n"
                      "• ⏳ <b>Avto-o'chirish</b> taymeridek bot qancha vaqtdan keyin avtomatik o'chishini belgilang.\n\n"
...
async def show_autohabar_panel(event: types.Message | types.CallbackQuery, user_id: int):
    try:
        ...
        # 2-MUOMMO HAL ETILDI: Forward rejim yoqilganda panelda xabar turi dinamik tarzda o'zgaradi!
        ...
        logging.exception(f"[Xato] show_autohabar_panel xatosi: {e}")
...
async def show_cabinet_panel(event: types.Message | types.CallbackQuery, user_id: int):
    ...
    ref_link = f"https://t.me/Auto_Xabar_Yuborish_Bot?start=ref_{real_user_id}"
...
async def show_guruhlar_menu(message: types.Message, user_id: int):
    ...
    inline_kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=btn_back, callback_data="back_to_welcome")]  # TUZATILDI: Welcome paneliga qaytadi!
    ])
    try:
...
async def show_profillar_settings(message: types.Message, user_id: int):
    ...
    buttons.append([InlineKeyboardButton(text=btn_back, callback_data="back_to_welcome")])  # TUZATILDI: Welcome paneliga qaytadi!
...
# UNIFIED SINGLE LANGUAGE CHANGER CALLBACK HANDLER
...
async def generate_control_panel_text(user_id: int):
    ...
    # 2-MUOMMO HAL ETILDI: Forward rejim yoqilganda xabar turi o'zgarmaslik xatosi hamda barcha tillarda to'g'ri ko'rsatish
    is_forward = user_data.get("is_forward_mode", False)
    if lang == "uz":
        msg_type = "Forward (Uzatish) 📤" if is_forward else "Matn/Media 📝"
    elif lang == "ru":
...
async def callback_toggle_sending(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if is_sending:
        ...
        db_users[user_id]["next_run_timestamp"] = datetime.now().timestamp() # Darhol jo'natish tsiklini yoqish
...
async def callback_user_statistika(callback_query: types.CallbackQuery):
    ...
    text = (
        ...
        f"🕒 Обновлено: <i>{datetime.now().strftime('%d.%m.%Y %H:%M')}</i>" if lang == "ru" else
        f"📊 <b>Your Broadcasting Statistics:</b>\n\n"
        f"✔️ Sent today: <b>{today_sent}</b>\n"
        f"🔄 Total sent: <b>{total_sent}</b>\n\n"
        f"🕒 Updated: <i>{datetime.now().strftime('%d.%m.%Y %H:%M')}</i>"
    )

    # 5-MUOMMO HAL ETILDI: Excel Hisobot yuklash imkoniyatini taqdim etamiz
    btn_export = "📥 Excel Hisobot (CSV)" if lang == "uz" else ("📥 Экспорт в Excel" if lang == "ru" else "📥 Export CSV")
    btn_back = "← " + ("Orqaga" if lang == "uz" else ("Назад" if lang == "ru" else "Back"))

    inline_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=btn_export, callback_data="export_stats_csv")],
...
async def callback_timer_setup(callback_query: types.CallbackQuery):
    ...
    text = (
        ...
        "⏱️ <b>Настройка таймера автовыключения</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Текущее время автовыключения: <b>{current_val_text}</b>\n\n"
        "Этот таймер позволяет автоматически остановить рассылку рекламы после истечения заданного времени. "
        "Это помогает избежать спам-блока, предотвращая бесконечную рассылку." if lang == "ru" else
        "⏱️ <b>Setup Auto-Off Timer</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Current auto-off delay: <b>{current_val_text}</b>\n\n"
        "This timer automatically turns off the broadcasting engine after a custom period. "
        "It prevents endless postings and minimizes Telegram spam limits."
    )
...
async def callback_set_timer_val(callback_query: types.CallbackQuery):
    ...
    if val_part == "inf":
        db_users[user_id]["auto_off_hours"] = None
        alert = "✓ Taymer o'chirildi (Cheksiz)!" if lang == "uz" else ("✓ Таймер отключен (Без ограничений)!" if lang == "ru" else "✓ Timer disabled (Unlimited)!")
    else:
        hours = int(val_part)
        db_users[user_id]["auto_off_hours"] = hours
        alert = f"✓ Avto-o'chirish {hours} soat qilib belgilandi!" if lang == "uz" else (f"✓ Автовыключение настроено на {hours} ч.!" if lang == "ru" else f"✓ Auto-off successfully set to {hours} hours!")
...
async def show_message_settings(message: types.Message, user_id: int):
    ...
    # 3-MUOMMO HAL ETILDI: Xabarni oldindan ko'rish (Preview) tugmasini sozlash
    btn_preview = "👁️ Xabarni ko'rish (Preview)" if lang == "uz" else ("👁️ Предпросмотр" if lang == "ru" else "👁️ Preview Post")
    ...
    try:
        await message.edit_text(textDetail, reply_markup=inline_kb, parse_mode="HTML")
    except Exception:
...
# ================= 👁️ FEATURE 3: PREVIEW AD POST (XABARNI OLDINDAN KO'RISH) =================

@router.callback_query(F.data == "preview_ad_post", StateFilter("*"))
async def callback_preview_post(callback_query: types.CallbackQuery):
    user_id = int(callback_query.from_user.id)
    ensure_user(user_id)
    user_data = db_users.get(user_id)
    
    text = user_data.get("reklama_matni", "Xabar matni kiritilmagan!")
    lang = user_data.get("lang", "uz") or "uz"
    
    # Ko'p nusxali xabarlar (Rotation) tekshiruvi previewda
    if "===" in text:
        posts = [p.strip() for p in text.split("===") if p.strip()]
        if posts:
            text = posts[0] + f"\n\n⚠️ <i>(Rotatsiyadagi {len(posts)} ta xabardan 1-si ko'rsatildi)</i>"

    # Watermark har doim biriktiriladi
    ...
    if buttons_data:
        preview_markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=btn["text"], url=btn["url"])] for btn in buttons_data
        ])

    await callback_query.answer("👁️ Reklamani oldindan ko'rish faollashdi", show_alert=False)
...
# ================= 📊 FEATURE 5: STATISTIKA EXCEL (CSV) EKSPORT TIZIMI =================
...
async def callback_export_stats_csv(callback_query: types.CallbackQuery):
    ...
    # Virtual StringIO orqali CSV yozamiz
    output = io.StringIO()
    writer = csv.writer(output)
    
    writer.writerow(["Parametr", "Qiymat"])
    writer.writerow(["Foydalanuvchi ID", user_id])
    writer.writerow(["Ulashtirilgan Akkaunt", user_data.get("active_phone", "Ulanmagan")])
    writer.writerow(["Bugun yuborilgan xabarlar", user_data.get("today_sent", 0)])
    writer.writerow(["Jami yuborilgan xabarlar", user_data.get("total_sent", 0)])
    writer.writerow(["Kutish vaqti (Interval)", f"{user_data.get('interval', 15)} daqiqa"])
    writer.writerow(["Avto-o'chirish taymeri", f"{user_data.get('auto_off_hours', 'Cheksiz')} soat"])
    writer.writerow(["Guruhlar tanlovi turi", user_data.get("groups_choice", "custom")])
    
    # Tanlangan guruhlarni jadvalga qo'shish
    selected_groups = user_data.get("selected_groups", [])
    writer.writerow([])
    writer.writerow(["Tanlangan guruh ID raqamlari"])
    for gid in selected_groups:
        writer.writerow([gid])

    # Faylni baytlar oqimiga o'tkazib yuboramiz
    bytes_data = output.getvalue().encode('utf-8-sig') # Excelda o'zbekcha harflar buzilmasligi uchun
    output.close()
    
    input_file = types.BufferedInputFile(bytes_data, filename=f"autohabar_report_{user_id}.csv")
    
    await callback_query.answer("📥 Hisobot fayli tayyorlanmoqda...", show_alert=False)
    
    try:
        await bot.send_document(
            chat_id=user_id,
            document=input_file,
            caption="📊 <b>Sizning AutoHabar Pro shaxsiy hisobotingiz (Excel/CSV)</b>\n\nFaylni yuklab olib, savdolaringizni tahlil qiling! 🚀",
            parse_mode="HTML"
        )
    except Exception as e:
        await callback_query.message.answer(f"❌ Faylni yuborishda xatolik: {e}")


# ================= 🚫 FEATURE 6: GURUHLAR QORA RO'YXATI (BLACKLIST) =================

@router.callback_query(F.data == "manage_blacklist", StateFilter("*"))
async def callback_manage_blacklist(callback_query: types.CallbackQuery, state: FSMContext = None):
    if state:
        await state.clear()
    user_id = int(callback_query.from_user.id)
    ensure_user(user_id)
    ...
    blacklist_gids = db_users[user_id]["blacklist"]
    
    text = (
        "🚫 <b>Guruhlar qora ro'yxati (Blacklist)</b>\n━━━━━━━━━━━━━━━━━━━━\n"
        f"Hozirda qora ro'yxatda: <b>{len(blacklist_gids)} ta</b> guruh IDsi bor.\n\n"
        "Ushbu guruhlarga bot tarqatish jarayonida xabar yubormay, avtomatik tashlab ketadi. Bu sizning ma'lum guruhlarga yuborishingizni cheklash uchun juda qulay! 😊\n\n"
        "✍️ <i>Yangi guruh IDlarini tahrirlash uchun pastdagi tugmani bosing va ularni har birini yangi qatordan kiriting.</i>"
    ) if lang == "uz" else (
        "🚫 <b>Черный список групп (Blacklist)</b>\n━━━━━━━━━━━━━━━━━━━━\n"
        f"В черном списке: <b>{len(blacklist_gids)}</b> ID групп.\n\n"
        "Бот автоматически пропустит эти группы при рассылке.\n\n"
        "✍️ <i>Для редактирования нажмите кнопку ниже и введите ID групп с новой строки.</i>" if lang == "ru" else
        "🚫 <b>Group Blacklist</b>\n━━━━━━━━━━━━━━━━━━━━\n"
        f"Currently blacklisted: <b>{len(blacklist_gids)} group(s)</b>.\n\n"
        "The bot will automatically ignore these groups during broadcasting cycle.\n\n"
        "✍️ <i>Click the button below to add/remove group IDs, entering each on a new line.</i>"
    )
    
    btn_edit = "✍️ Ro'yxatni tahrirlash" if lang == "uz" else ("✍️ Редактировать список" if lang == "ru" else "✍️ Edit Blacklist")
    btn_clear = "🗑 Tozalash" if lang == "uz" else ("🗑 Очистить" if lang == "ru" else "🗑 Clear")
    btn_back = "← Orqaga" if lang == "uz" else ("← Назад" if lang == "ru" else "← Back")
    
    kb = [
        [InlineKeyboardButton(text=btn_edit, callback_data="edit_blacklist"),
         InlineKeyboardButton(text=btn_clear, callback_data="clear_blacklist")],
        [InlineKeyboardButton(text=btn_back, callback_data="back_to_panel")]
    ]
    await callback_query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb), parse_mode="HTML")
    await callback_query.answer()

@router.callback_query(F.data == "edit_blacklist", StateFilter("*"))
async def callback_edit_blacklist(callback_query: types.CallbackQuery, state: FSMContext):
    user_id = int(callback_query.from_user.id)
    lang = db_users[user_id].get("lang", "uz") or "uz"
    
    prompt = (
        "✍️ <b>Qora ro'yxatga qo'shiladigan guruh ID raqamlarini kiriting:</b>\n\n"
        "Har birini yangi qatordan yozing (Masalan:\n<code>-1002166666115\n-1002414392202</code>):"
    ) if lang == "uz" else (
        "✍️ <b>Введите ID групп для черного списка:</b>\n\n"
        "Каждый ID вводите с новой строки (Например:\n<code>-1002166666115\n-1002414392202</code>):" if lang == "ru" else
        "✍️ <b>Enter group IDs for Blacklist:</b>\n\n"
        "Enter each ID on a new line (e.g.:\n<code>-1002166666115\n-1002414392202</code>):"
    )
...
async def state_blacklist_received(message: types.Message, state: FSMContext):
    if await check_and_redirect_if_menu(message, state):
        return
    ...
    db_users[user_id]["blacklist"] = valid_ids
    save_db()
    
    lang = db_users[user_id].get("lang", "uz") or "uz"
    success = f"✅ <b>Qora ro'yxat yangilandi! Jami: {len(valid_ids)} ta guruh ID muvaffaqiyatli saqlandi.</b>" if lang == "uz" else (
        f"✅ <b>Черный список обновлен! Всего сохранено {len(valid_ids)} ID групп.</b>" if lang == "ru" else
        f"✅ <b>Blacklist updated! Total {len(valid_ids)} group IDs saved.</b>"
    )
...
async def callback_clear_blacklist(callback_query: types.CallbackQuery):
    ...
    await callback_manage_blacklist(callback_query, None)
...
async def menu_groups_handler(message: types.Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    ensure_user(user_id)
    await show_guruhlar_menu(message, user_id)
...
async def send_reklama_message(client, chat_id, user_data, user_id):
    ...
    # 9-VAZIFA: Watermark matni bepul va pullik foydalanuvchilar uchun strictly Uzbek tilida biriktirilishi
    watermark = "\n\n@Auto_Xabar_Yuborish_Bot orqali yuborildi"
    
    raw_text = user_data.get("reklama_matni", "")
    
    # 2-XUSUSIYAT: Ko'p nusxali xabarlar rotatsiyasi (=== split) (Multi-Post Rotation!)
    if "===" in raw_text:
        posts = [p.strip() for p in raw_text.split("===") if p.strip()]
        if posts:
            raw_text = random.choice(posts)  # Har safar aylanma usulda tasodifiy birini tanlaydi
            
    text_to_send = raw_text + watermark
    ...
    if user_data.get("is_pro") and user_data.get("is_forward_mode") and user_data.get("forward_msg_id") and user_data.get("forward_chat_id"):
        try:
            # Forward xabar uzatilganda uning matni pastidan watermark qo'shib bo'lmasligi tufayli ketidan alohida watermark yuboriladi
            await client.forward_messages(chat_id, user_data.get("forward_msg_id"), user_data.get("forward_chat_id"))
            await client.send_message(chat_id, watermark)
        except Exception as e:
        ...
        if photo_path and os.path.exists(photo_path):
            try:
                await client.send_message(chat_id, text_to_send, file=photo_path, buttons=telethon_buttons)
            except Exception as e:
                logging.warning(f"Rasm bilan yuborishda muammo: {e}. Faqat matn ko'rinishida yuborilmoqda.")
                await client.send_message(chat_id, text_to_send, buttons=telethon_buttons)
        else:
            await client.send_message(chat_id, text_to_send, buttons=telethon_buttons)
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            for g_id in guruhlar:
                ...
                # 6-XUSUSIYAT: Qora ro'yxat (Blacklist) tekshiruvi (Bypass!)
                blacklist = db_users[user_id].get("blacklist", [])
                if int(g_id) in [int(x) for x in blacklist]:
                    logging.info(f"[Blacklist] Guruh {g_id} qora ro'yxatda bo'lgani sababli tashlab ketildi.")
                    continue
                ...
                try:
                    ...
                    # 1-XUSUSIYAT: Tasodifiy Kechikish (Random Delay) — Anti-Ban Himoyasi (12 dan 25 soniyagacha tasodifiy!)
                    random_sleep = random.randint(12, 25)
                    logging.info(f"[Anti-Ban] Keyingi guruh oldidan {random_sleep} soniya kutilmoqda...")
...
async def callback_back_to_welcome_handler(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not user_data.get("active_phone"):
        inline_keyboard_buttons.append([InlineKeyboardButton(text=get_text(user_id, "btn_add_acc"), callback_data="add_account")])
    
    inline_keyboard_buttons.append([InlineKeyboardButton(text="❌ Yopish" if user_data.get("lang") == "uz" else ("❌ Закрыть" if user_data.get("lang") == "ru" else "❌ Close"), callback_data="close_menu")])
    
    try:
        await callback_query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=inline_keyboard_buttons), parse_mode="HTML")
    except Exception:
        await callback_query.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=inline_keyboard_buttons), parse_mode="HTML")
    await callback_query.answer()


# ================= REAL TIME LOGIN ACCOUNT SELECTION =================
...
# ================= DEPOSIT & CUSTOM INTERVAL HANDLERS =================
...
async def callback_deposit_balance(callback_query: types.CallbackQuery, state: FSMContext):
    """Hisobni to'ldirish drayveri!"""
    await state.clear()
    user_id = int(callback_query.from_user.id)
    ensure_user(user_id)
    lang = db_users[user_id].get("lang", "uz") or "uz"
    
    text = LOCALIZATION[lang]["deposit_title"].format(
        user_id=user_id,
    ...
    kb = [[InlineKeyboardButton(text=btn_back, callback_data="back_to_kabinet")]]
    
    try:
        await callback_query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb), parse_mode="HTML")
    except Exception:
        await callback_query.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb), parse_mode="HTML")
    await callback_query.answer()

@router.callback_query(F.data == "custom_interval", StateFilter("*"))
async def callback_custom_interval_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    """Intervalni qo'lda kiritish prompti!"""
    await state.clear()
    user_id = int(callback_query.from_user.id)
    ensure_user(user_id)
    lang = db_users[user_id].get("lang", "uz") or "uz"
...
async def state_custom_interval_received(message: types.Message, state: FSMContext):
    """Qo'lda kiritilgan intervalni asinxron tekshirib saqlovchi handler!"""
    # Anti-Trap redirection
    if await check_and_redirect_if_menu(message, state):
        return
    ...
    try:
        ...
        # Interval sozlash menyusiga qaytarish
        user_data = db_users.get(user_id)
